...
if __name__ == '__main__':
    ...
    print(elastic.evaluateRecommendations(URM_test=urm_test, at=10))
    ...
    print("elastic cbf scores")
    elastic_cbf = Linear_combination_scores_recommender(urm_csr=urm_train,
                                                        rec_dictionary=elastic_cbf_dictionary)

    recommender_elastic = CFW_D_Similarity_Linalg(URM_train=urm_train, ICM=icm_complete, S_matrix_target=elastic.W_sparse)
    recommender_elastic.fit(damp_coeff=0.1, add_zeros_quota=1.0, topK=50)
