...
class MatrixFactorization_Cython(Recommender, Incremental_Training_Early_Stopping):
    ...
    def fit(self, epochs=300, batch_size = 1000, num_factors=10,
        ...
        from Recommenders.ML_recommenders.MatrixFactorization.Cython.MatrixFactorization_Cython_Epoch import MatrixFactorization_Cython_Epoch
        ...
        print("Mean W elements = " + str(self.W.mean()))
        W_array = self.W.ravel()
        W_non_zero_array = W_array[W_array.nonzero()]
        W_sparse = 1 - (W_non_zero_array.shape[0] / W_array.shape[0])
        print("Sparsity W=" + str(W_sparse))

        print("Mean H elements = " + str(self.H.mean()))
        H_array = self.H.ravel()
        H_non_zero_array = H_array[H_array.nonzero()]
        H_sparse = 1 - (H_non_zero_array.shape[0] / H_array.shape[0])
        print("Sparsity H=" + str(H_sparse))
...
if __name__ == '__main__':
    ...
    evaluator = SequentialEvaluator(URM_test_list=urm_validation, ignore_users=utility.user_to_neglect(), cutoff_list=[10])
    recommender = MatrixFactorization_Cython(URM_train=urm_train)


    recommender.fit(epochs=1000, user_reg=0.000001, positive_reg=0.000001, negative_reg=0.1, learning_rate=0.01,\
                    validation_every_n=100, num_factors=50, evaluator_object=evaluator)